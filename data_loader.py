...
def load_dataset(name):
    ...
    # UAV-Cyber-Physical is soted pre-split as .npy (same as cicids)
    # Load directly to match Paper 1's preprocessing
    if name == "uav_cyber":
        base = config.DATA_ROOT / "UAV-Cyber-Physical" / "processed"
        X_train = np.load(base / "X_train.npy")
        X_test = np.load(base / "X_test.npy")
        y_train = pd.read_csv(base / "y_train.csv").squeeze().to_numpy()
        y_test = pd.read_csv(base / "y_test.csv").squeeze().to_numpy()
        feature_names = pd.read_csv(base / "feature_names.csv").squeeze().tolist()
        return X_train, X_test, y_train, y_test, feature_names
